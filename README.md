...
- [WebSocket 保活机制](#websocket-保活机制ws-keeper)
- [多会员礼包机制](#多会员礼包机制)
- [登录 Code 获取方式](#登录-code-获取方式)
...
## WebSocket 保活机制（ws-keeper）

### 背景：主容器频繁重启的问题

QQ 农场游戏通过 **WebSocket 长连接**与服务器通信，每条连接使用一次性的 `code`（授权令牌）建立。  
如果主容器重启，原来的 WebSocket 连接断开，`code` 会立即失效，需要重新扫码登录——这对多账号管理来说极不友好。

### 解决方案：独立保活容器

项目引入了独立的 `ws-keeper` 服务（独立 Docker 容器），专门负责维护与游戏服务器的 WebSocket 长连接，与主服务完全解耦：

```
手机/QQ 扫码
      │ code（授权令牌）
      ▼
  主服务（8081）──POST /connect──▶ ws-keeper（4001）
                                       │
                              持久化 WS 连接（含心跳）
                                       │
                              游戏服务器（wss://...）

主服务重启 ──不影响──▶ ws-keeper 保持连接，code 永不失效
```
...
主服务调用 `POST /connect { accountId, code }`，ws-keeper 与游戏服务器建立 WebSocket 连接并完成登录握手：
- 发送 `LoginRequest`（含设备信息、客户端版本等）
- 服务器返回 `LoginReply`，记录 `gid`（玩家游戏ID）、等级、金币等基本信息
- 登录成功后启动心跳定时器

**2. 心跳保活**

登录成功后，ws-keeper 每 **30 秒**向游戏服务器发送一次 `Heartbeat` 请求，并带随机 0~10 秒抖动，避免多账号心跳同时触发造成瞬时峰值：
...
若超过 **90 秒**未收到心跳响应，ws-keeper 会主动触发重连，无需人工干预。

**3. 断线自动重连**

- WS 连接意外断开（非 code 失效）→ **5 秒后自动重连**，直到连接恢复
- code 失效（服务器返回 HTTP 400）→ 清除 code，停止自动重连，并通过 SSE 通知主服务触发重新登录流程

**4. 请求队列（防心跳挤占）**

天王模式下多个账号并发发送大量业务请求，可能将心跳包挤出队列导致掉线。ws-keeper 对每个账号维护一个独立的**串行发送队列**：

```
账号A的业务请求 → [队列A] → 串行发出 → 心跳不被挤占
账号B的业务请求 → [队列B] → 串行发出 → 账号间完全并行
```

同一账号的请求逐个等待响应后再发下一个，不同账号之间互不干扰，彻底解决天王模式下的高并发掉线问题。

**5. WASM 加密协议**

游戏服务器要求对业务消息体进行加密。ws-keeper 集成 WASM 加解密模块，在发送前自动对 Protobuf body 进行加密处理，服务端返回的响应为明文无需解密。

**6. SSE 实时推送**

游戏服务器主动下发的事件（如农场成熟通知 `LandsNotify`、任务完成通知等）通过 **Server-Sent Events（SSE）** 实时推送给主服务，主服务订阅 `/subscribe` 端点获取：

```
游戏服务器推送 → ws-keeper 接收 → SSE → 主服务 → 立即触发巡田
```

这使得农场操作无需轮询，有推送即响应，大幅降低服务器资源消耗。

**7. 幂等连接**

若账号已登录且 code 未变，重复调用 `POST /connect` 不会重置连接（返回 `alreadyConnected: true`），避免主服务多次重启导致不必要的重连。

### ws-keeper HTTP API

| 接口 | 方法 | 说明 |
|------|------|------|
| `/connect` | POST | `{ accountId, code }` 建立/更新账号连接 |
| `/disconnect` | POST | `{ accountId }` 断开并删除账号连接 |
| `/status` | GET | `?accountId=xxx` 查询单账号连接状态 |
| `/status/all` | GET | 查询所有账号连接状态列表 |
| `/send` | POST | `{ accountId, service, method, body }` 发送游戏消息 |
| `/subscribe` | GET | `?accountId=xxx` SSE 订阅游戏服务器推送通知 |

---

## 多会员礼包机制

本项目支持多种游戏内会员/订阅制度的礼包自动领取，无需手动操作，每日 0 点后自动在下次巡田循环中触发：

### QQ 会员每日礼包

若账号绑定了 QQ 会员（超级会员/红钻等），系统每日自动检测并领取游戏内 **QQ 会员专属每日礼包**：

- 调用 `QQVipService.GetDailyGiftStatus` 查询今日是否可领取
- 若 `can_claim = true`，立即调用 `ClaimDailyGift` 领取
- 领取成功后记录日志，展示奖励内容（金币/经验/点券等）
- 当日已领取（错误码 `1021002`）自动跳过，不重复请求
- 每次检测间隔 **10 分钟**，防止频繁请求

### 月卡礼包

若账号购买了游戏月卡，系统每日自动领取 **月卡每日补给**：

- 调用 `MallService.GetMonthCardInfos` 查询当前所有月卡信息
- 对所有 `can_claim = true` 的月卡逐个调用 `ClaimMonthCardReward` 领取（支持同时持有多张月卡）
- 领取成功后记录每张月卡的奖励明细
- 当日全部领完后标记为"今日已完成"，不再重复检测

### 多账号独立管理

- 每个账号的会员礼包状态完全独立，互不干扰
- Web 面板 → 日志页 → 筛选 `module=task` 可查看所有礼包领取记录
- 领取失败时有错误日志，便于排查账号会员资格变更等情况

---

## 登录 Code 获取方式

游戏通过一次性的授权 `code` 登录，获取方式有以下两种：

### 方式一：QQ 扫码登录（推荐）

> ⚠️ **注意**：目前 QQ 官方小程序的扫码登录入口已有限制，直接扫码可能无法成功。推荐使用下方的代理重定向方式获取 code。

1. 面板 → 账号管理 → 新增账号 → 填写账号名称
2. 点击"生成二维码"，用 QQ 扫描二维码
3. QQ 在完成授权后，会向游戏服务器发送一个包含 `code` 参数的回调请求
4. 如果系统能拦截到该回调，账号状态会自动变为"在线"

### 方式二：代理重定向抓取 Code（最稳定）

当手机和服务器处于同一网络环境，或手机能访问到服务器时，可以通过代理工具将游戏服务器的回调重定向到 `sniff9988.py` 监听服务，实现全自动 code 注入。

#### 原理

QQ 扫码授权成功后，会向以下地址发起回调请求：

```
https://gate-obt.nqf.qq.com/...?code=xxxxx&uin=xxxxx
```

通过在手机上配置代理（推荐 [ProxyPin](https://github.com/wanghongenpin/proxypin)），将 `gate-obt.nqf.qq.com/*` 的请求重定向到 `sniff9988.py`，即可自动捕获 code 并注入到挂机服务。

#### 配置步骤

**第一步：启动 sniff9988.py（服务器端）**

```bash
# 在主容器内或宿主机启动（监听 9988 端口）
python3 sniff9988.py

# 或 Docker 环境下在主容器内执行
docker exec -it youxian-qq-farm-bot-ui python3 /app/sniff9988.py
```

`sniff9988.py` 在 `9988` 端口开启 HTTP 服务，等待接收 code 回调：
- 接收到 `?code=...&uin=...` 请求后，自动转发给主服务 `/api/code-capture` 接口
- 同一 code + 账号 60 秒内只转发一次，防止 QQ 重试导致重复建账号
- 支持通过路径区分账号：访问 `/账号名` 时，code 会注入到对应的账号
- 运行日志写入 `/tmp/sniff9988.out`，便于排查问题

**第二步：在手机上配置 ProxyPin 重定向规则**

1. 安装并打开 [ProxyPin](https://github.com/wanghongenpin/proxypin)（Android / iOS 均支持）
2. 进入 **重写 → 请求重写 → 新建规则**，填写：

| 字段 | 值 |
|------|-----|
| 名称 | `QQ农场code获取` |
| URL | `ANY` → `gate-obt.nqf.qq.com/*` |
| 行为 | **重定向** |
| 重定向到 | `http://你的服务器IP:9988/账号名` |

3. 启用该规则，开启 ProxyPin 代理

**第三步：扫码**

- 在 QQ 内打开农场小程序，正常扫描面板生成的二维码
- QQ 授权完成后，回调请求会被 ProxyPin 重定向到 `sniff9988.py`
- `sniff9988.py` 自动将 code 注入到指定账号，账号状态自动变为"在线"

#### 多账号区分

通过在重定向 URL 末尾加上账号名来区分不同账号：

| 重定向地址 | 注入目标账号 |
|-----------|-------------|
| `http://服务器IP:9988/账号A` | 账号名为"账号A"的账号 |
| `http://服务器IP:9988/账号B` | 账号名为"账号B"的账号 |
| `http://服务器IP:9988/` | 默认账号（`DEFAULT_USERNAME`） |
...
#### 账号管理页
- 新增账号（填写名称）→ 生成二维码 → 扫码/代理注入 code 登录
- 查看每个账号的详细配置：
...
牧场图鉴是内置的独立小游戏模块，与 QQ 农场主线并行运行，数据存储在 `data/ranch.json`，与农场数据完全隔离。

### 游戏概述

牧场以**保种**为核心玩法：解锁稀有动物品种 → 喂食养殖 → 繁殖 → 完成保种任务 → 解锁更高难度品种，形成完整的收集养成链路。

### 牧场金币

牧场金币是牧场系统专用货币，**唯一来源是农场出售果实**：

- 每次出售果实获得的游戏金币，会**等额同步**到牧场金币余额
- 牧场金币用于购买饲料、药品等牧场道具
- 余额显示在牧场面板顶部

### 品种图鉴

所有可养殖品种按**品类**（猪类、禽类、牛类等）和**繁育难度**（1★~5★）分类：
...
解锁品种（赠送公母一对）
       │
       ▼
   定期喂食（每天 N 份饲料，按品类对应饲料类型）
       │
       ├─── 健康度 < 30% → 无法繁殖，需先喂食恢复
       ├─── 患病 → 需使用对应药品治疗后才能繁殖
       │
       ▼
   尝试繁殖（有繁殖冷却期，通常 3 天/次）
       │
       ├─── 繁殖成功（成功率 70% + 答题/笔记加成，最高 95%）
       │         ├─── 累计成功次数 +1
       │         ├─── 10% 概率触发疾病
       │         └─── 累计达到目标次数（通常 4 次）→ 保种成功
       │
       └─── 繁殖失败 → 冷却后重试
```
...
每个品种配有专属知识题库，答题全部正确可获得**繁育速度加成**：

- 答题加成最高 **+50%**（全部答对时增加，上限 50%）
- 答题冷却时间：**2 分钟**
- 加成生效后，繁殖成功率从基础 70% 提升，最高至 95%

### 好友经验笔记（无上限加成）

保种成功后可获得该品种的《保种经验》书籍，可赠送给好友：

- 好友收到书籍并在邮件箱领取后，该品种繁育速度加成 **+25%**（无上限，可叠加）
- 书籍每本只能赠出一次，赠出后自身书籍消耗
- 互赠笔记是提升繁殖效率的最快方式

**加成叠加规则：**

```
总繁育加成 = 答题加成（上限 50%）+ 笔记加成（无上限，每本 +25%）
最终繁殖成功率 = min(70% + 总加成%, 95%)
```

### 疾病与治疗

繁殖时有 **10%** 的概率触发疾病，患病动物无法繁殖：

- 不同品类动物对应不同疾病类型（猪流感、禽流感、牛病毒等）
- 需使用对应品类的**专效药品**治疗（通用药品对部分品类无效）
- 治疗成功后健康度恢复 +30%
- 牧场面板会高亮显示患病状态，方便快速处理
...
|------|------|
| 自购物品 | 使用牧场金币购买的饲料、药品 |
| 收到的书籍 | 好友赠送的《保种经验》书籍（领取后显示在此处，并自动叠加加成） |
...
| 操作 | 说明 |
|------|------|
| 设置/增加金币 | 直接修改账号牧场金币余额 |
| 强制解锁品种 | 忽略前置条件直接解锁（不赠送额外公母） |
| 补充库存 | 直接向账号库存中添加饲料/药品 |
| 重置答题冷却 | 立即恢复答题资格，无需等待 2 分钟冷却 |
| 修改品种参数 | 运行时覆盖繁殖冷却天数、目标次数、每日饲料需求 |
| 修改商品价格 | 运行时覆盖饲料/药品价格 |
...
│   │       ├── ranch.js           # 牧场图鉴小游戏（养殖/繁殖/图鉴）
│   │       ├── analytics.js       # 作物收益分析（分析页数据）
│   │       ├── antiafk.js         # 防挂/防检测逻辑
│   │       ├── monthcard.js       # 月卡每日礼包自动领取
│   │       ├── qqvip.js           # QQ 会员每日礼包自动领取
...
│   ├── index.js                   # 保活服务主逻辑（持久化 WS 连接、心跳、重连、请求队列）
│   ├── utils/
│   │   └── crypto-wasm.js         # WASM 加密模块（游戏消息加解密）
│   └── Dockerfile
│
├── sniff9988.py                   # QQ 扫码 Code 抓取转发工具（Python 3，监听 9988 端口）
...
4. 建议使用 ProxyPin 代理重定向方式，直接通过扫码已不稳定（见[登录 Code 获取方式](#登录-code-获取方式)）

**Q: QQ 直接扫码登录不成功？**

当前 QQ 官方小程序对扫码登录回调有限制，直接扫码获取 code 可能不稳定甚至失效。  
推荐使用 **ProxyPin 代理重定向** 方式：配置规则将 `gate-obt.nqf.qq.com/*` 重定向到你的 `sniff9988.py` 服务，扫码后 code 会自动注入账号，无需手动填写。
...
这是一个 Python 辅助工具，在 `9988` 端口开启 HTTP 服务，用于配合代理工具（ProxyPin 等）拦截 QQ 扫码登录的授权回调。  
收到 `?code=...&uin=...` 请求后，自动将 code 转发给主服务的 `/api/code-capture` 接口完成账号登录。  
支持通过 URL 路径区分账号：`/账号名` → 注入到对应账号。

**Q: 主容器重启后账号会断线吗？**

不会。ws-keeper 容器独立运行，主容器重启不影响 ws-keeper 中已建立的 WebSocket 连接和 code。  
主容器重启后会重新连接到 ws-keeper，账号状态自动恢复，无需重新登录。
...
- 适配 WASM 加密协议：[sulimu2/qq-farm-bot](https://github.com/sulimu2/qq-farm-bot)
