    ...
    return {
        ...
        getStatus: (accountRef) => {
            ...
            return {
                ...
                // 强制用 store 最新值覆盖 worker status_sync 里可能滞后的 automation
        ...
        useBagItem: (accountRef, itemId, count) => callWorkerApi(resolveAccountRefId(accountRef), 'useBagItem', itemId, count),
        ...
        getTheftKingStatus: (accountRef) =>
            callWorkerApi(resolveAccountRefId(accountRef), 'getTheftKingStatus'),

        setTheftKingConfig: (accountRef, cfg) =>
            callWorkerApi(resolveAccountRefId(accountRef), 'setTheftKingConfig', cfg),
        ...
        clearLogs: (accountRef) => {
            const accountId = resolveAccountRefId(accountRef);
            if (!accountId) return { accountId: '', cleared: 0 };
            const accId = String(accountId);
            let cleared = 0;
            // 清空 globalLogs 中属于该账号的条目
