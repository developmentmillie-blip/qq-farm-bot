const { tryRestore, saveCurrent } = require('./persist-stats');
...
function setStatsAccountId(accountId) { _currentAccountId = String(accountId || ''); }
...
function initStats(gold, exp, coupon = 0) {
    ...
    if (_currentAccountId) {
        const saved = tryRestore(_currentAccountId);
        if (saved) {
            if (saved.operations) {
                for (const [k, v] of Object.entries(saved.operations)) {
                    if (operations[k] !== undefined) operations[k] = Number(v) || 0;
                }
            }
            session.goldGained = Number(saved.sessionGoldGained) || 0;
            session.expGained = Number(saved.sessionExpGained) || 0;
            session.couponGained = Number(saved.sessionCouponGained) || 0;
            initialState.gold = g - session.goldGained;
            initialState.exp = e - session.expGained;
            initialState.coupon = c - session.couponGained;
        }
    }
}
