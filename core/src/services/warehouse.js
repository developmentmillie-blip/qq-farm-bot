...
const { isAutomationOn, getPlantBlacklist, getFruitSellBlacklist } = require('../models/store');
...
async function getBagDetail() {
    ...
    // 同 id 按 count 降序，再按 uid 升序
    rows.sort((a, b) => {
        if (a.id !== b.id) return a.id - b.id;
        if (b.count !== a.count) return b.count - a.count;
        return a.uid - b.uid;
    });
    return { totalKinds: rows.length, items: rows };
}
...
async function sellAllFruits() {
    ...
    try {
        ...
        // 果实出售黑名单（按账号独立配置）
        const _acctId = String(process.env.FARM_ACCOUNT_ID || '').trim();
        const sellBlacklist = _acctId
            ? (getFruitSellBlacklist ? getFruitSellBlacklist(_acctId) : [])
            : (getPlantBlacklist ? getPlantBlacklist('__fruit_sell__') : []);
        const sellBlacklistNames = new Set((sellBlacklist || []).map(s => String(s).trim().toLowerCase()));
        ...
        for (const item of items) {
            ...
            if (isFruitItemId(id) && count > 0) {
                const fruitName = String(getFruitName(id) || '').trim();
                if (sellBlacklistNames.size > 0 && sellBlacklistNames.has(fruitName.toLowerCase())) {
                    skippedNames.push(`${fruitName}x${count}`);
                    continue;
                }
                toSell.push(item);
                names.push(`${fruitName}x${count}`);
            }
        }
        if (skippedNames.length > 0) {
            log('仓库', `跳过不出售果实: ${skippedNames.join(', ')}`, {
                module: 'warehouse',
                event: 'sell_skip_blacklist',
                result: 'skip',
            });
        }
