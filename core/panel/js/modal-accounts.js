...
// 切换扫码平台 qq / wx
function setQrScanPlatform(plat) {
    qrScanPlatform = plat || 'qq';
    // 更新按钮高亮
    document.querySelectorAll('.qr-platform-btn').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.plat === qrScanPlatform);
    });
    // 更新提示文字
    const hint = $('qr-hint');
    const jumpBtn = $('btn-qr-open-url');
    if (hint) {
        if (qrScanPlatform === 'wx') {
            hint.innerHTML = '<span style="color:#000;">先点击生成二维码</span><br><span style="color:red;font-weight:bold;">有封微信、盗号风险，后果自负。</span>';
        } else {
            hint.innerHTML = '用 <strong>QQ</strong> 扫码，选择「QQ经典农场」授权';
        }
    }
    if (jumpBtn) {
        jumpBtn.textContent = qrScanPlatform === 'wx' ? '在微信中打开' : '跳转QQ登录';
    }
    // 备注名 placeholder
    const nameQr = $('acc-name-qr');
    if (nameQr) {
        nameQr.placeholder = qrScanPlatform === 'wx' ? '不填则默认使用微信昵称' : '不填则默认使用QQ昵称';
    }
}
...
async function generateQRCode() {
    ...
    try {
        const apiUrl = qrScanPlatform === 'wx' ? '/api/qr/wx/create' : '/api/qr/create';
        ...
        if (result.ok && result.data) {
            if (qrScanPlatform === 'wx') {
                currentQRCode = result.data.uuid;
            } else {
            ...
            // 生成后同步平台提示
            setQrScanPlatform(qrScanPlatform);
            const statusEl = $('qr-status');
            if (statusEl) {
                statusEl.textContent = qrScanPlatform === 'wx' ? '请用微信扫码...' : '请用QQ扫码...';
...
async function openQRCodeLoginUrl() {
    ...
    if (qrScanPlatform === 'wx') {
        // 微信 deep link：唤起微信扫码/打开链接
        const b64 = (typeof btoa === 'function')
            ? btoa(unescape(encodeURIComponent(currentLoginUrl)))
            : '';
        const wxDeepLink = b64
            ? `weixin://dl/business/?ticket=${encodeURIComponent(currentLoginUrl)}`
            : '';
        // 先尝试 weixin:// scheme，失败则直接跳链接（用户手机微信扫）
        let switched = false;
        const onVis = () => { if (document.hidden) switched = true; };
        document.addEventListener('visibilitychange', onVis);
        window.location.href = wxDeepLink || currentLoginUrl;
        setTimeout(() => {
            document.removeEventListener('visibilitychange', onVis);
            if (!switched) {
                // weixin:// 未成功唤起，直接打开链接让用户在微信里手动打开
                window.open(currentLoginUrl, '_blank');
            }
        }, 1200);
        return;
    }
...
function startQRCheck() {
    ...
    qrCheckInterval = setInterval(async () => {
        ...
        try {
            const checkApiUrl = qrScanPlatform === 'wx' ? '/api/qr/wx/check' : '/api/qr/check';
            const checkBody = qrScanPlatform === 'wx'
                ? JSON.stringify({ uuid: currentQRCode })
                : JSON.stringify({ code: currentQRCode });
            ...
            if (result.ok && result.data) {
                ...
                if (status) {
                    if (result.data.status === 'Wait') {
                        status.textContent = qrScanPlatform === 'wx' ? '请用微信扫码...' : '请用QQ扫码...';
                        ...
                        const qqNickname = result.data.nickname || result.data.nick || '';
                        if (!accName) {
                            accName = qqNickname || (result.data.wxid ? result.data.wxid : (result.data.uin ? String(result.data.uin) : '扫码账号'));
                        }
                        ...
                        try {
                            ...
                            const payload = {
                                ...
                                platform: qrScanPlatform === 'wx' ? 'wx' : 'qq',
                                uin: qq,
                                qq,
...
document.querySelectorAll('.qr-platform-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        setQrScanPlatform(btn.dataset.plat);
    });
});
...
window.editAccount = (id) => {
    ...
    // 按账号原有平台初始化扫码平台
    qrScanPlatform = acc.platform === 'wx' ? 'wx' : 'qq';
    setQrScanPlatform(qrScanPlatform);
...
window.deleteAccount = async (id) => {
    if (confirm('确定停用该账号？\n停用后账号将暂停运行，2天后自动删除。管理员可立即删除。')) {
        const ret = await api('/api/accounts/' + id, 'DELETE');
        if (!ret) return;
        if (ret.softDeleted) {
            showToast('账号已停用，2天后将自动删除');
        } else {
            const sid = String(id);
...
window.forceDeleteAccount = async (id) => {
    if (confirm('确定立即删除该账号？此操作不可撤销。')) {
        const ret = await api('/api/accounts/' + id + '/force', 'DELETE');
        if (!ret) return;
        const sid = String(id);
        accounts = accounts.filter(a => String(a.id) !== sid);
        if (String(currentAccountId || '') === sid) currentAccountId = null;
        renderAccountSelector();
        renderAccountManager();
        renderLogFilterOptions();
        await loadAccounts();
...
window.clearAllPendingAccounts = async () => {
    const ret = await api('/api/accounts/pending/all', 'DELETE');
    if (!ret) return;
    const cleared = ret.cleared || 0;
    await loadAccounts();
    renderAccountManager();
    renderAccountSelector();
    renderLogFilterOptions();
    await pollAccountLogs();
    showToast(`已清除 ${cleared} 个待删除账号`);
};
