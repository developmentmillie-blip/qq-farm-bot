...
async function loadAccounts() {
    return runDedupedRequest('loadAccounts', async () => {
        ...
        if (list && list.accounts) {
            ...
            // 如果是管理员,加载用户列表（完成后重渲账号管理以显示归属用户）
            if (currentUser && currentUser.role === 'admin' && $('page-accounts').classList.contains('active')) {
                await loadUsers();
                renderAccountManager(); // users 加载完毕后重新渲染，确保归属标签正确显示
            }
...
async function pollStatus(options = {}) {
    ...
    return runDedupedRequest(key, async () => {
        ...
        if (!pendingAutomationKeys.has('smart_farm_schedule')) $('auto-smart-farm-schedule').checked = !!auto.smart_farm_schedule;
